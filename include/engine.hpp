...
class Shaders {
    public:
        Shaders() {
                TEXTUREDvertexShaderSrc = readShaderFromFile((TexturedShadersPath + tvName).c_str());
                TEXTUREDfragmentShaderSrc = readShaderFromFile((TexturedShadersPath + tfName).c_str());

                FLATvertexShaderSrc = readShaderFromFile((FlatShadersPath + fvName).c_str());
                FLATfragmentShaderSrc = readShaderFromFile((FlatShadersPath + ffName).c_str());
...
class Render {
    public:
        ...
        void Transform(GLuint shaderProgram) { // where tf is this is used why is it even here
